(() => {
  ...
  function renderLeaderboard(loadError = false) {
    const entries = Array.isArray(state.leaderboard)
      ? state.leaderboard.map(entry => ({ ...entry, mode: entry.mode || 'timed' }))
      : [];
    const timedEntries = entries.filter(entry => entry.mode === 'timed');
    const infiniteEntries = entries.filter(entry => entry.mode === 'infinite');
    renderLeaderboardList(state.leaderboardLists.timed, timedEntries, entry => formatDuration(entry.duration || 0), loadError);
    renderLeaderboardList(state.leaderboardLists.infinite, infiniteEntries, () => '—', loadError);
    updateLeaderboardTabs();
  }

  function renderLeaderboardList(container, entries, metaFormatter, loadError) {
    if (!container) {
      return;
    }
    if (loadError) {
