(() => {
  // application/maze-runner/src/game-legacy.js
  (function() {
    "use strict";
    ...
    async function fetchStages({ isTeacher, difficulty }) {
      ...
      if (isTeacher) {
        const resp2 = await apiRequest(`/api/teachers/maze-runner/stages?difficulty=${encodeURIComponent(diff)}`, { method: "GET" });
        return await mrJson(resp2);
      }
    ...
    async function fetchStageDetail({ isTeacher, stageId }) {
      ...
      if (isTeacher) {
        const resp2 = await apiRequest(`/api/teachers/maze-runner/stages/${encodeURIComponent(id)}`, { method: "GET" });
        return await mrJson(resp2);
      }
      const sid = getPublicStudentIdFromPlayers();
      const pwd = getPublicStudentPassword();
    ...
    function openCreateStageModal({ root, difficulty, stage, onCreated }) {
      ...
      const clampCfgToBoard = () => {
        ...
        cfg.blacks = (Array.isArray(cfg.blacks) ? cfg.blacks : []).map((b) => ({ type: String(b?.type || "P").toUpperCase(), ...clamp(b) })).filter((x) => inBounds(x.r, x.c, rows, cols));
        cfg.portals = (Array.isArray(cfg.portals) ? cfg.portals : []).map((p) => ({ a: p?.a ? clamp(p.a) : null, b: p?.b ? clamp(p.b) : null })).filter((p) => p.a && p.b && !(p.a.r === p.b.r && p.a.c === p.b.c));
      };
      ...
      const renderCellContent = (r, c) => {
        ...
        if (portalExit(portalsMap, r, c)) return `<span aria-hidden="true" style="font-size:18px;">\u{1F300}</span>`;
      ...
      <div class="vcp-modal-backdrop" id="mrCreateBackdrop" role="presentation">
        <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Create stage" style="width: calc(100vw - 40px); max-width: 1300px;">
          <div class="vcp-modal-header">
            ...
            <button id="mrCreateClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
          </div>
            ...
            <div style="display:grid; grid-template-columns: 420px 1fr; gap:14px; margin-top:12px; align-items:start;">
              ...
              <div>
                ...
                <div class="mr-card" style="margin-top:10px; background:#ffffff;">
                  ...
                  <div class="mr-muted" style="margin-top:10px;">
                    Start = white piece. Goals = G1, G2, ... (in order). Portal = \u{1F300} (place in pairs).
                  </div>
                </div>
              </div>
            </div>
    ...
    function renderStageList({ difficulty, stages }) {
      const diff = normalizeDiff(difficulty);
      const list = Array.isArray(stages) ? stages : [];
      const pickerHidden = !!stages?.__hideDiffPicker;
      return `
      <div class="mr-toolbar">
        <div></div>
        <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
    ...
    function normalizeStageConfig(raw) {
      ...
      let goals = Array.isArray(cfg?.goals) ? cfg.goals.map((x) => ({ r: Number(x?.r), c: Number(x?.c) })).filter((x) => Number.isFinite(x.r) && Number.isFinite(x.c) && inBounds(x.r, x.c, rows, cols)) : [];
      if (!goals.length && legacyGoal) goals = [legacyGoal];
      if (!goals.length) goals = [clampRc(null, { r: 0, c: cols - 1 })];
      const rocks = Array.isArray(cfg?.rocks) ? cfg.rocks.map((x) => ({ r: Number(x?.r), c: Number(x?.c) })).filter((x) => Number.isFinite(x.r) && Number.isFinite(x.c) && inBounds(x.r, x.c, rows, cols)) : [];
      const blacks = Array.isArray(cfg?.blacks) ? cfg.blacks.map((x) => ({ type: String(x?.type || "P").trim().toUpperCase(), r: Number(x?.r), c: Number(x?.c) })).filter((x) => Number.isFinite(x.r) && Number.isFinite(x.c) && inBounds(x.r, x.c, rows, cols)) : [];
      const portals = Array.isArray(cfg?.portals) ? cfg.portals.map((p) => ({
        a: p?.a ? { r: Number(p.a.r), c: Number(p.a.c) } : null,
        b: p?.b ? { r: Number(p.b.r), c: Number(p.b.c) } : null
      })).filter(
        (p) => p.a && p.b && Number.isFinite(p.a.r) && Number.isFinite(p.a.c) && inBounds(p.a.r, p.a.c, rows, cols) && Number.isFinite(p.b.r) && Number.isFinite(p.b.c) && inBounds(p.b.r, p.b.c, rows, cols) && !(p.a.r === p.b.r && p.a.c === p.b.c)
      ) : [];
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          if (isRock) {
            ...
            inner = `<span aria-hidden="true" style="font-size:18px;">\u{1F300}</span>`;
          }
      ...
      <div class="vcp-modal-backdrop" id="mrRulesBackdrop" role="presentation">
        <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Rules" style="width: calc(100vw - 40px); max-width: 980px;">
          <div class="vcp-modal-header">
            ...
            <button id="mrRulesClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
          </div>
          <div class="vcp-modal-body">
      ...
      <div class="vcp-modal-backdrop" id="mrAttackBackdrop" role="presentation">
        <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Attack warning" style="width: calc(100vw - 40px); max-width: 720px;">
          <div class="vcp-modal-header">
            ...
            <button id="mrAttackClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
          </div>
          <div class="vcp-modal-body">
      ...
      <div class="vcp-modal-backdrop" id="mrSuccessBackdrop" role="presentation">
        <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Success" style="width: calc(100vw - 40px); max-width: 720px;">
          <div class="vcp-modal-header">
            ...
            <button id="mrSuccessClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
          </div>
          <div class="vcp-modal-body">
    ...
    window.initMazeRunner = async function initMazeRunner() {
      ...
      const loadCompleted = () => {
        try {
          ...
          ui.progress.completed = /* @__PURE__ */ new Set();
        }
      };
      const saveCompleted = () => {
        try {
      ...
      async function loadStageList() {
        ...
        try {
          ...
          ui.stage.stages.forEach((s) => {
            try {
              s.__isComplete = isStageComplete(s?.id);
            } catch {
            }
          });
          ui.stage.view = "list";
      ...
      function bindSettingsHandlers() {
        ...
        const setHint = (txt) => {
          if (hint) hint.textContent = String(txt || "");
        };
