(() => {
  // application/monster-fight-shell/src/game-legacy.js
    ...
    container.innerHTML = `
        <div class="game-screen">
            <div class="character-selection-header">
                <h2>\u{1F409} Monster Fight - Character Selection</h2>
    ...
    container.innerHTML = `
        <div class="game-screen">
            <div class="battle-header">
                <h2>\u{1F409} Monster Fight - Level ${gameState.currentLevel}</h2>
            ...
            <div class="battle-layout-horizontal">
                <div class="monsters-section">
                    ...
                    <div class="monsters-grid">
                        ${gameState.monsters && gameState.monsters.length > 0 ? gameState.monsters.map((monster) => renderMonsterCard(monster)).join("") : "<p>No monsters yet. Initialize battle to start.</p>"}
                    </div>
                </div>
      ...
      return `
                ...
                ${!player.isAlive ? `
                    <div class="revive-section">
                        <button class="btn btn-sm btn-warning" onclick="showReviveModal('${player.studentId}')">\u{1F4AB} Revive</button>
                    </div>
                ` : ""}
  ...
  function addIconToName(name, type, id) {
    ...
    if (type === "player") {
      ...
      const icon = cls && cls.emoji || CLASS_ICON_MAP[id] || "\u{1F9D1}";
      return `${icon} ${name}`;
    }
    if (type === "monster") {
      const icon = monsterIconMap[id] || "\u{1F47E}";
      return `${icon} ${name}`;
    }
    return name;
  }
