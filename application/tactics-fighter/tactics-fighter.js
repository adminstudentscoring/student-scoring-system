(() => {
  // application/tactics-fighter/src/debug.js
  ...
  // application/tactics-fighter/src/settings.js
  function createTfSettingsHandlers({ apiRequest, tfJson }, { ui, isTeacher, publicStudentId, publicStudentPassword }) {
  ...
  // application/tactics-fighter/src/app-legacy.js
  var TF = window.__TacticsFighterCore;
  if (!TF) {
    ...
    window.initTacticsFighter = async function initTacticsFighter() {
      ...
      const ui = {
        ...
        expanded: {
          cat: /* @__PURE__ */ new Set(),
          topic: /* @__PURE__ */ new Set(),
          subtopic: /* @__PURE__ */ new Set(),
          puzzlesLoaded: /* @__PURE__ */ new Set()
        },
        puzzlesBySubtopic: /* @__PURE__ */ new Map(),
        puzzlePageBySubtopic: /* @__PURE__ */ new Map(),
        teacher: {
          bucket: (() => {
            try {
              return normalizeBucketKey(localStorage.getItem("tacticsFighterPracticeBucket") || "beginner");
            } catch {
              return "beginner";
            }
          })(),
          ...
          // { categories: [] }
          view: "bucket",
          // bucket | categories | topics | subtopics | puzzles
          categoryId: null,
          topicId: null,
          subtopicId: null,
          puzzlesAll: [],
      ...
      function isPseudoLegalMove(board, side, fromCoord, toCoord) {
        ...
        if (dst) {
          ...
          if (dstIsWhite === isWhite) return false;
        }
        const dr = tr.r - fr.r;
        const dc = tr.c - fr.c;
        ...
        if (t === "k") {
          ...
          if (wantSide === "w" && fromSq === "e1" && (toSq === "g1" || toSq === "c1")) {
            if (toSq === "g1") {
              const f1 = coordToRc("f1");
              const g1 = coordToRc("g1");
              const h1 = coordToRc("h1");
              if (!f1 || !g1 || !h1) return false;
              return !b[f1.r][f1.c] && !b[g1.r][g1.c] && b[h1.r][h1.c] === "R";
            } else {
              const b1 = coordToRc("b1");
              const c1 = coordToRc("c1");
              const d1 = coordToRc("d1");
              const a1 = coordToRc("a1");
          ...
          if (wantSide === "b" && fromSq === "e8" && (toSq === "g8" || toSq === "c8")) {
            if (toSq === "g8") {
              const f8 = coordToRc("f8");
              const g8 = coordToRc("g8");
              const h8 = coordToRc("h8");
              if (!f8 || !g8 || !h8) return false;
              return !b[f8.r][f8.c] && !b[g8.r][g8.c] && b[h8.r][h8.c] === "r";
            } else {
              const b8 = coordToRc("b8");
              const c8 = coordToRc("c8");
              const d8 = coordToRc("d8");
              const a8 = coordToRc("a8");
        ...
        <div class="vcp-modal-backdrop" id="tfPromoBackdrop" role="presentation">
          <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Promotion" style="width: calc(100vw - 40px); max-width: 420px;">
            <div class="vcp-modal-header">
              ...
              <button id="tfPromoClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
            </div>
      ...
      function renderStudentPuzzles(puzzles, page, pageSize, total) {
        ...
        <div style="margin-top:12px; display:flex; align-items:center; justify-content:space-between; gap:10px;">
          <div class="tf-muted">Page ${p} / ${totalPages} \xB7 ${Number(total || 0)} puzzles</div>
      ...
      async function openStudentRunnerModal() {
        ...
        ui.student.runner = {
          ...
          selectedFrom: null,
          lastVerdict: null,
          // 'correct' | 'incorrect' | null (persistent until next submit)
          ...
          side: startSide,
          history: [],
          // entries: { fen, board, side, movesUciLen, movesSanLen }
      ...
      `;
        ...
        const fadeDuring = async (fn) => {
          ...
          if (fadeHost) {
            requestAnimationFrame(() => {
              try {
                fadeHost.classList.remove("is-out");
              } catch {
              }
            });
            await wait(220);
        ...
        function currentPuzzle() {
          const total2 = Math.max(0, Number(ui.student.total || 0));
          if (!total2) return null;
          const abs = Math.max(0, Math.min(total2 - 1, Math.trunc(Number(ui.student.runner?.absIndex || 0))));
          const { page, idx } = absIndexToPage(abs);
          const pageData2 = ui.student.puzzlePages?.[String(page)];
          const list = Array.isArray(pageData2?.puzzles) ? pageData2.puzzles : Array.isArray(ui.student.puzzles) ? ui.student.puzzles : [];
        ...
        async function resetRunnerToAbsIndex(nextAbs) {
          const total2 = Math.max(0, Number(ui.student.total || 0));
          if (!total2) return false;
          const abs = Math.max(0, Math.min(total2 - 1, Math.trunc(Number(nextAbs || 0))));
          const { page, idx } = absIndexToPage(abs);
          ui.student.page = page;
          const pageData2 = await studentEnsurePuzzlePage(page);
          ui.student.puzzles = Array.isArray(pageData2?.puzzles) ? pageData2.puzzles : [];
          const pz = ui.student.puzzles[idx];
          if (!pz) return false;
          const startFen2 = String(pz?.fen || "").trim();
          const startBoard2 = parseFenToBoard(startFen2);
          const startSide2 = fenSideToMove(startFen2);
          ui.student.runner.absIndex = abs;
          ui.student.runner.movesUci = [];
          ui.student.runner.movesSan = [];
          ui.student.runner.selectedFrom = null;
          ui.student.runner.startFen = startFen2;
          ui.student.runner.fen = startFen2;
          ui.student.runner.board = startBoard2 || Array.from({ length: 8 }, () => Array(8).fill(""));
          ui.student.runner.side = startSide2;
          ...
          ui.student.runner.playerSide = startSide2;
          ui.student.runner.orientation = startSide2 === "b" ? "black" : "white";
          return true;
        }
        ...
        function renderRunner() {
          ...
          const total2 = Math.max(0, Number(ui.student.total || 0));
          const abs = Math.max(0, Math.min(Math.max(0, total2 - 1), Math.trunc(Number(ui.student.runner?.absIndex || 0))));
          if (meta) meta.textContent = `Puzzle ${abs + 1} / ${total2 || 0}`;
          ...
          if (fb) {
            ...
            if (showCompleted || showIncorrect) {
              ...
              const btnHtml = showCompleted ? `<button type="button" class="btn btn-primary" data-stu-feedback-next="1">Next</button>
                 <button type="button" class="btn btn-secondary" data-stu-feedback-tryagain="1">Try again</button>` : `<button type="button" class="btn btn-secondary" data-stu-feedback-redo="1">Redo</button>`;
        ...
        async function submitMoveAndReply() {
          ...
          try {
            ...
            if (out.completed) {
              ...
              try {
                ui.student.verdictByPuzzleId[String(pz.id)] = "correct";
              } catch {
              }
              try {
                delete ui.student.tryAgainByPuzzleId[String(pz.id)];
              } catch {
              }
              ...
              try {
                ui.student.verdictByPuzzleId[String(pz.id)] = "incorrect";
              } catch {
              }
        ...
        const clearDragHover = () => {
          try {
            drag.hoverEl?.classList?.remove("is-drop-target");
          } catch {
          }
          drag.hoverEl = null;
        };
        ...
        modal.addEventListener("pointerdown", (ev) => {
          ...
          removeGhost();
          try {
            sq.setPointerCapture?.(ev.pointerId);
          } catch {
          }
        });
        ...
        modal.addEventListener("click", (ev) => {
          ...
          if (t.closest("[data-stu-feedback-next]")) {
            (async () => {
              const total2 = Math.max(0, Number(ui.student.total || 0));
              if (!total2) return;
              const cur = Math.max(0, Math.min(total2 - 1, Math.trunc(Number(ui.student.runner?.absIndex || 0))));
        ...
        function renderRunner() {
          ...
          if (fb) {
            ...
            if (showCorrectDone || showIncorrect) {
              ...
              const btnHtml = showCorrectDone ? `<button type="button" class="btn btn-primary" data-tea-feedback-next="1">Next</button>` : `<button type="button" class="btn btn-secondary" data-tea-feedback-redo="1">Redo</button>`;
              fb.classList.toggle("is-ok", isOk);
              fb.classList.toggle("is-err", !isOk);
              fb.innerHTML = `
              <div class="tf-stu-feedback-box">
      ...
      function setBuilderBucket(bucket) {
        try {
          localStorage.setItem("tacticsFighterBuilderBucket", String(bucket || "beginner"));
        } catch {
        }
      }
      ...
      function setBulkPvPlies(v) {
        ...
        try {
          localStorage.setItem("tacticsFighterBulkPvPlies", String(out));
        } catch {
        }
        return out;
      }
        ...
        <div class="vcp-modal-backdrop" id="tfMoveCatBackdrop" role="presentation">
          <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Move category" style="width: calc(100vw - 40px); max-width: 720px;">
            <div class="vcp-modal-header">
              ...
              <button id="tfMoveCatClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
            </div>
            <div class="vcp-modal-body">
        ...
        <div class="vcp-modal-backdrop" id="tfSubMsgBackdrop" role="presentation">
          <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Subtopic message" style="width: calc(100vw - 40px); max-width: 900px;">
            <div class="vcp-modal-header">
              ...
              <button id="tfSubMsgClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
            </div>
            <div class="vcp-modal-body">
        ...
        tfDbgTeacherPractice("teacherFetchTree:response", {
          ...
          firstSubtopicSample: (() => {
            try {
              ...
              return s0 ? { id: s0.id, name: s0.name, puzzleCount: s0.puzzleCount, keys: Object.keys(s0) } : null;
            } catch {
              return null;
            }
          })()
      ...
      async function teacherShowCategories(bucket) {
        ...
        try {
          ...
          try {
            localStorage.setItem("tacticsFighterPracticeBucket", ui.teacher.bucket);
          } catch {
          }
      ...
      function renderTeacherPuzzles(subtopicName, puzzlesAll, page, pageSize) {
        ...
        <div style="margin-top:12px; display:flex; align-items:center; justify-content:space-between; gap:10px;">
          <div class="tf-muted">Page ${p} / ${totalPages} \xB7 ${total} puzzles</div>
          <div style="display:flex; gap:10px;">
          ...
          <div class="vcp-modal-backdrop" id="tfChooseStuBackdrop" role="presentation">
            <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Choose students" style="width: calc(100vw - 40px); max-width: 980px;">
              <div class="vcp-modal-header">
                ...
                <button id="tfChooseStuClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
              </div>
              <div class="vcp-modal-body">
          ...
          const renderList = () => {
            ...
            for (const s of shown) {
              ...
              <div style="min-width:0;">
                ...
                <div class="tf-muted" style="margin-top:2px;">ID: ${escapeHtml(sid)}${inClass ? " \xB7 In Class View" : ""}</div>
              </div>
            `;
        ...
        <div class="vcp-modal-backdrop" id="tfLinksBackdrop" role="presentation">
          <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Student links" style="width: calc(100vw - 40px); max-width: 980px;">
            <div class="vcp-modal-header">
              ...
              <button id="tfLinksClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
            </div>
            <div class="vcp-modal-body">
...
`).join("\n");
          try {
            ...
            toastShow("ok", "Copied.", { autoHideMs: 2e3 });
          } catch {
            try {
      ...
      async function studentLoadTree(bucket) {
        ...
        try {
          localStorage.setItem("tacticsFighterPracticeBucket", ui.student.bucket);
        } catch {
        }
      ...
      async function studentApplyDeepLinkIfAny() {
        ...
        try {
          ...
          try {
            localStorage.removeItem("tacticsFighterDeepLink");
          } catch {
          }
          try {
      ...
      const activateMode = (m) => {
        ...
        if (nm === "home" && !isTeacher && publicStudentId) {
          (async () => {
            try {
              const main2 = document.getElementById("tfMain");
              toastShow("loading", "Loading...");
              if (main2) main2.innerHTML = `<div class="tf-muted"></div>`;
              const stats = await studentFetchStats(publicStudentId, null, publicStudentPassword);
              ui.student.stats = stats;
              toastHide();
              if (main2) main2.innerHTML = renderHome(stats);
            } catch (e) {
              const main2 = document.getElementById("tfMain");
              toastShow("err", e?.message || String(e));
              if (main2) main2.innerHTML = `<div class="tf-builder-msg err" style="display:block;">${escapeHtml(e?.message || String(e))}</div>`;
            }
          })();
        }
        ...
        if (nm === "builder") {
          ...
          tree?.addEventListener("click", async (ev) => {
            ...
            if (addTopicBtn) {
              ...
              if (!name) return;
              try {
                await builderCreateTopic(cid, name);
                await builderRefresh();
              } catch (e) {
                showBuilderMsg("err", e?.message || String(e));
              }
            ...
            if (addSubBtn) {
              ...
              if (!name) return;
              try {
                await builderCreateSubtopic(tid, name);
                await builderRefresh();
              } catch (e) {
                showBuilderMsg("err", e?.message || String(e));
              }
            ...
            if (renCatBtn) {
              ...
              if (!name) return;
              try {
                await builderRenameCategory(cid, name);
                await builderRefresh();
              } catch (e) {
                showBuilderMsg("err", e?.message || String(e));
              }
            ...
            if (delCatBtn) {
              ...
              if (!ok) return;
              try {
                await builderDeleteCategory(cid);
                await builderRefresh();
              } catch (e) {
                showBuilderMsg("err", e?.message || String(e));
              }
            ...
            if (renTopicBtn) {
              ...
              if (!name) return;
              try {
                await builderRenameTopic(tid, name);
                await builderRefresh();
              } catch (e) {
                showBuilderMsg("err", e?.message || String(e));
              }
            ...
            if (renSubBtn) {
              ...
              if (!name) return;
              try {
                await builderRenameSubtopic(sid, name);
                await builderRefresh();
              } catch (e) {
                showBuilderMsg("err", e?.message || String(e));
              }
            ...
            if (delSubBtn) {
              ...
              if (!ok) return;
              try {
                await builderDeleteSubtopic(sid);
                await builderRefresh();
              } catch (e) {
                showBuilderMsg("err", e?.message || String(e));
              }
        ...
        <div class="vcp-modal-backdrop" id="tfPuzzleDetailBackdrop" role="presentation">
          <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Puzzle detail" style="width: calc(100vw - 40px); max-width: 1400px;">
            <div class="vcp-modal-header">
              ...
              <button id="tfPuzzleDetailClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
            </div>
            <div class="vcp-modal-body">
      ...
      `;
        ...
        function renderAnswers() {
          ...
          const html = lines.length ? lines.map((ln) => {
            ...
            const score = scoreObj && Object.prototype.hasOwnProperty.call(scoreObj, "mate") ? `mate ${Number(scoreObj.mate) || 0}` : `cp ${Number(scoreObj.cp) || 0}`;
            const pv = formatPvWithMoveNumbersHtml(fen, ln?.pvSan);
            const fallback = Array.isArray(ln?.pvUci) ? escapeHtml(ln.pvUci.join(" ")) : "";
            return `<div class="tf-line"><div class="tf-line-title">#${escapeHtml(mp)} \xB7 ${escapeHtml(score)}</div><div class="tf-line-meta">${pv || fallback}</div></div>`;
        ...
        const setEditMsg = (s) => {
          if (editMsg) editMsg.textContent = String(s || "");
        };
        const openEdit = () => {
        ...
        <div class="vcp-modal-backdrop" id="tfAddPuzzleBackdrop" role="presentation">
          <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Add puzzles" style="width: calc(100vw - 40px); max-width: 1600px;">
            <div class="vcp-modal-header">
              ...
              <button id="tfAddPuzzleClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
            </div>
            <div class="vcp-modal-body">
              <div class="tf-modal-grid">
                <div>
      ...
      `;
        ...
        const selectedAnswerMultiPv = /* @__PURE__ */ new Set();
        function formatPvWithMoveNumbers(fen, pvSan) {
          const parts = String(fen || "").trim().split(/\s+/);
          const side2 = parts[1] === "b" ? "b" : "w";
          const fullmove = Math.max(1, Number(parts[5] || 1) || 1);
          const moves = Array.isArray(pvSan) ? pvSan.map(String).filter(Boolean) : [];
          if (!moves.length) return "";
          const lines = [];
        ...
        function renderPalette() {
          ...
          paletteEl.innerHTML = pieces.map((p) => {
            ...
            const inner = src ? `<img class="tf-piece-img" alt="" src="${escapeHtml(src)}">` : escapeHtml(PIECE_UNICODE[p] || p);
        ...
        function setEngineOut(html) {
          if (engineOutEl) engineOutEl.innerHTML = html;
        }
              ...
              <div class="tf-line">
                <div style="display:flex; justify-content:space-between; gap:10px; align-items:flex-start;">
                  <label style="display:flex; gap:10px; align-items:center; cursor:pointer;">
                    ...
                    <div class="tf-line-title">#${escapeHtml(mp)} \xB7 ${escapeHtml(score)}</div>
                  </label>
                </div>
                <div class="tf-line-meta">${pv || fallback}</div>
              </div>
        ...
        async function openManualAnswerModal2(startFen) {
          ...
          <div class="vcp-modal-backdrop" id="tfManualBackdrop" role="presentation">
            <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Manual Input" style="width: calc(100vw - 40px); max-width: 1200px;">
              <div class="vcp-modal-header">
                ...
                <button id="tfManualClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
              </div>
              <div class="vcp-modal-body">
        ...
        `;
          ...
          const tryPromotion = async (board2, from, to) => {
            if (!needsPawnPromotion(board2, from, to)) return "";
            const fr = coordToRc(from);
            const pawn = fr ? board2?.[fr.r]?.[fr.c] || "" : "";
        ...
        host.querySelector("#tfManualInputBtn")?.addEventListener("click", async () => {
          try {
            applyFenText();
            const fen = String(fenInput?.value || "").trim();
            const out = await openManualAnswerModal2(fen);
            const pvUci = Array.isArray(out?.pvUci) ? out.pvUci : [];
            const pvSan = Array.isArray(out?.pvSan) ? out.pvSan : [];
            if (!pvUci.length) throw new Error("No moves");
            manualLine = {
        ...
        <div class="vcp-modal-backdrop" id="tfBulkBackdrop" role="presentation">
          <div class="vcp-modal" role="dialog" aria-modal="true" aria-label="Bulk Import" style="width: calc(100vw - 40px); max-width: 1400px;">
            <div class="vcp-modal-header">
              ...
              <button id="tfBulkClose" class="vcp-modal-close" type="button" aria-label="Close">\xD7</button>
            </div>
            <div class="vcp-modal-body">
      ...
      `;
        ...
        const close = () => {
          try {
            if (typeof host.__tfBulkCleanup === "function") host.__tfBulkCleanup();
          } catch {
          }
          try {
            host.remove();
        ...
        const updateCounts = () => {
          ...
          if (countsEl) countsEl.textContent = `Total: ${total} \xB7 Saved: ${saved} \xB7 Done: ${done} \xB7 Pending: ${pending} \xB7 Error: ${err}`;
        ...
        const renderList = () => {
          ...
          listEl.innerHTML = entries.map((e, i) => {
            ...
            const badge = e.status === "saved" ? "Saved" : e.status === "done" ? "Done" : e.status === "running" ? "Running" : e.status === "error" ? "Error" : "Pending";
            return `
        ...
        const stopClipboard = () => {
          ...
          lastClipboardText = "";
          try {
            if (clipStartBtn) clipStartBtn.disabled = false;
          } catch {
          }
          try {
            if (clipStopBtn) clipStopBtn.disabled = true;
          } catch {
          }
        };
...
${text}` : text;
        ...
        clipStartBtn?.addEventListener("click", () => {
          ...
          lastClipboardText = "";
          try {
            if (clipStartBtn) clipStartBtn.disabled = true;
          } catch {
          }
          try {
            if (clipStopBtn) clipStopBtn.disabled = false;
          } catch {
          }
      ...
      root.addEventListener("click", (e) => {
          ...
          if (ghostStartBtn) {
            (async () => {
              try {
                ...
                ui.student.puzzles = puzzles.map((p) => ({ ...p, completed: false }));
        ...
        if (isTeacher) {
          const backBtn2 = target.closest("[data-tea-back]");
          if (backBtn2) {
            const dest = String(backBtn2.getAttribute("data-tea-back") || "").trim();
            if (dest === "buckets") {
          ...
          const catBtn2 = target.closest("[data-tea-cat]");
          if (catBtn2) {
            const cid = String(catBtn2.getAttribute("data-tea-cat") || "").trim();
          ...
          const topicBtn2 = target.closest("[data-tea-topic]");
          if (topicBtn2) {
            const tid = String(topicBtn2.getAttribute("data-tea-topic") || "").trim();
            const cat = teacherFindCategoryById(ui.teacher.categoryId);
          ...
          const subBtn2 = target.closest("[data-tea-subtopic]");
          if (subBtn2) {
            const sid = String(subBtn2.getAttribute("data-tea-subtopic") || "").trim();
            const cat = teacherFindCategoryById(ui.teacher.categoryId);
            const topic = teacherFindTopicById(cat, ui.teacher.topicId);
          ...
          const pageBtn2 = target.closest("[data-tea-page]");
          if (pageBtn2) {
            const dir = String(pageBtn2.getAttribute("data-tea-page") || "").trim();
            if (dir === "prev" || dir === "next") return void teacherChangePuzzlePage(dir);
            return;
          }
          const startBtn2 = target.closest("[data-tea-start]");
          if (startBtn2) {
            (async () => {
              try {
                ...
                toastShow("err", e2?.message || String(e2));
              }
            })();
            return;
          }
