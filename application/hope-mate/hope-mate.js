(() => {
  // application/hope-mate/src/game-legacy.js
  (function() {
    let BOARD_SIZE = 5;
    let FILES = ["a", "b", "c", "d", "e"];
    let RANKS = [1, 2, 3, 4, 5];
    ...
    const PIECE_POOL_WHITE = ["Q", "R", "B", "N", "K", "P"];
    const PIECE_POOL_BLACK = ["q", "r", "b", "n", "p"];
    const MODES = [
    ...
    function getPracticeConfig(levelNumber) {
      ...
      const blackExtraCount = Math.max(0, lvl - 1);
    ...
    function getPlayers() {
      const players = Array.isArray(window.hopeMatePlayers) ? window.hopeMatePlayers : safeJsonParse(localStorage.getItem(STORAGE.players)) || [];
      return players;
    }
    ...
    function pieceImagePath(piece) {
      ...
      switch (up) {
        case "K":
          return `/application/pieces/${colorPrefix}King.png`;
        case "Q":
          return `/application/pieces/${colorPrefix}Queen.png`;
        case "R":
          return `/application/pieces/${colorPrefix}Rook.png`;
        case "B":
          return `/application/pieces/${colorPrefix}Bishop.png`;
        case "N":
          return `/application/pieces/${colorPrefix}Knight.png`;
        case "P":
          return `/application/pieces/${colorPrefix}Pawn.png`;
        default:
          return null;
      }
    }
    ...
    function buildAttackMap(board, side) {
      const attacks = /* @__PURE__ */ new Set();
    ...
    function validateWhitePlacementConstraints(baseBoard, blackKingIdx, placements) {
    ...
    function randomPuzzle(cfg) {
      ...
      const maxTries = BOARD_SIZE === 8 ? fixedWhitePieces && fixedWhitePieces.length === 1 ? 2e4 : 3e3 : 4e3;
      for (let attempt = 0; attempt < maxTries; attempt++) {
        const boardBase = buildEmptyBoard();
        const isStageRook = BOARD_SIZE === 8 && fixedWhitePieces && fixedWhitePieces.length === 1 && String(fixedWhitePieces[0]).toUpperCase() === "R" && (blackExtraCount === 2 || blackExtraCount === 3);
        ...
        if (isStageRook) {
          ...
          const trap1 = xyToIdx(corner.x + sx, corner.y);
          const trap2 = xyToIdx(corner.x + sx, corner.y + sy);
          boardBase[trap1] = "n";
          boardBase[trap2] = "n";
        ...
        const whitePieces = fixedWhitePieces && fixedWhitePieces.length > 0 ? fixedWhitePieces.map((p) => String(p).toUpperCase()) : [sample(PIECE_POOL_WHITE), sample(PIECE_POOL_WHITE)];
        const squares = [];
        ...
        const tryPair = (idx1, idx2) => {
          ...
          const assignmentOptions = p1 === p2 ? [[{ piece: p1, idx: idx1 }, { piece: p2, idx: idx2 }]] : [
        ...
        if (pieceCount === 1) {
          ...
          const samples = Math.min(5e3, squares.length * 10);
          for (let t = 0; t < samples; t++) {
            const picks = [];
    ...
    const state = {
      screen: "home",
      // 'home' | 'practiceSelect' | 'practiceGame' | 'challengeSelect' | 'challengeGame'
      practiceLevel: 1,
      challenge: {
        active: false,
        durationSec: 60,
        ...
        level: 1,
        solvedInLevel: 0,
        // 0..1 (level up every 2 solved)
        totalSolved: 0
      },
      puzzle: null,
      board: null,
      placed: [],
      // indices for each white piece slot
      selectedPieceSlot: 0,
    ...
    function placePiece(slot, idx) {
      ...
      if (!piece) return;
      if (state.puzzle.black[idx]) return;
    ...
    function confirm() {
      ...
      if (mate) {
        ...
        const msg2 = gained ? "Correct! Checkmate. +1 point." : "Correct! Checkmate. (No points because you already failed this puzzle.)";
        setStatus(msg2, "success");
        openResult("correct", msg2);
        ...
        if (gained === 1) {
          const player = getSinglePlayer();
          if (player?.id) {
            submitHopeMateTotalScore(String(player.id), state.totalScore).then((entries) => {
              state.leaderboard.entries = entries;
              state.leaderboard.loading = false;
              state.leaderboard.error = null;
              render();
      ...
      if (isStalemate(state.board)) {
        ...
        const msg2 = "Incorrect. Stalemate is a failure. Redo (no points for this puzzle).";
        setStatus(msg2, "error");
        openResult("incorrect", msg2);
        render();
        return;
      }
      state.attemptsFailed = true;
    ...
    function render() {
      ...
      if (state.screen === "challengeGame") {
        const cfg2 = challengeConfigForLevel(state.challenge.level);
        const levelLabel2 = `Level ${cfg2.level} (${cfg2.boardSize}\xD7${cfg2.boardSize}, black pieces: ${cfg2.blackExtraCount})`;
        ...
        <div class="hope-mate-shell">
          <div class="hope-mate-topbar">
            <div class="hope-mate-title-wrap">
              <div class="hope-mate-title">\u2728 Hope Mate</div>
              <div class="hope-mate-subtitle">Challenge Mode \u2014 ${escapeHtml(levelLabel2)} \u2014 Place 2 pieces to checkmate the black king (black to move).</div>
            </div>
            <div class="hope-mate-meta">
              <div><strong>Student:</strong> ${escapeHtml(playerName)}</div>
            ...
            <div class="hm-modal hm-result-modal" role="dialog" aria-modal="true" aria-label="Hope Mate Result">
              ...
              <div class="hm-modal-body">
                ...
                <div class="hm-result-actions">
                  ${state.ui.resultKind === "correct" ? `<button id="hmResultNext" class="btn btn-primary" type="button">Next</button>` : `<button id="hmResultRedo" class="btn btn-primary" type="button">Redo</button>`}
                </div>
              </div>
            </div>
          </div>
      ...
      const cfg = getPracticeConfig(state.practiceLevel);
      const levelLabel = `Level ${cfg.level} (${cfg.boardSize}\xD7${cfg.boardSize}, black pieces: ${cfg.blackExtraCount})`;
      root.innerHTML = `
      <div class="hope-mate-shell">
        <div class="hope-mate-topbar">
          <div class="hope-mate-title-wrap">
