(() => {
  // application/my-own-app/src/eatwhat-legacy.js
  (function() {
    "use strict";
    ...
    const ui = {
      ...
      editPersonId: null,
      editFoodId: null,
      foodSlotDraft: /* @__PURE__ */ new Set(),
      foodSlotEditDraft: /* @__PURE__ */ new Set(),
      canvasCssPx: 320,
      canvasDpr: 1
    };
    const MEAL_SLOTS = [
    ...
    function normalizeState(s) {
      ...
      const foods2 = foods.map((f) => ({
        id: String(f?.id || uid()),
        name: String(f?.name || "").trim(),
        tags: Array.isArray(f?.tags) ? f.tags.map((t) => String(t || "").trim()).filter(Boolean) : [],
        slots: Array.isArray(f?.slots) ? f.slots.map((t) => String(t || "").trim()).filter(Boolean) : defaultFoodSlots()
      })).filter((f) => !!f.name);
    ...
    function getCandidates() {
      ...
      return foods.filter((f) => foodAllowedForMeal(f, mk)).filter((f) => foodAllowedForExclusions(f, ex));
    }
    ...
    async function spinOnce() {
      ...
      if (hint) {
        ...
        hint.textContent = `Meal: ${meal} \xB7 People: ${pplLabel}`;
      }
      ui.spinning = false;
    ...
    function renderAll() {
      ...
      if (hint) {
        ...
        const meal = mealLabel(ui.state.selectedMealSlot);
        hint.textContent = `Meal: ${meal} \xB7 Candidates: ${cands.length} / ${foods.length}`;
      }
    }
    function bind() {
      document.querySelectorAll(".ew-tabbtn").forEach((b) => {
