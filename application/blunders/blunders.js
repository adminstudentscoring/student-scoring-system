(() => {
  // application/blunders/src/game-legacy.js
  (function() {
    ...
    function renderSidebar() {
      return window.BlundersStudent?.renderSidebar?.() || `<aside class="bl-sidebar"><div class="bl-side-title">\u{1F4A5} Blunders</div></aside>`;
    }
    function renderDebugBlock() {
      return window.BlundersStudent?.renderDebugBlock?.() || ``;
    }
    function openHomePracticeModal() {
      return window.BlundersStudent?.openHomePracticeModal?.();
    }
    function startPracticeFromHome(key) {
      return window.BlundersStudent?.startPracticeFromHome?.(key);
    }
    function renderHomePage() {
      return window.BlundersStudent?.renderHomePage?.() || `<div class="bl-card">Student module not loaded.</div>`;
    }
    function renderBlunderPage() {
      return window.BlundersStudent?.renderBlunderPage?.() || `<div class="bl-card">Student module not loaded.</div>`;
    }
    function renderReviewPage() {
      return window.BlundersStudent?.renderReviewPage?.() || `<div class="bl-card">Student module not loaded.</div>`;
    }
    function renderStudentMasterGamePage() {
      return window.BlundersStudent?.renderStudentMasterGamePage?.() || `<div class="bl-card">Student module not loaded.</div>`;
    }
    function renderSettingsPage() {
      return window.BlundersStudent?.renderSettingsPage?.() || `<div class="bl-card">Student module not loaded.</div>`;
    }
      ...
      <div class="bl-modal-backdrop" id="blModalBackdrop" role="presentation">
        <div class="bl-modal" role="dialog" aria-modal="true" aria-label="${escapeHtml(title)}">
          <div class="bl-modal-header">
            ...
            <button class="bl-modal-close" type="button" id="blModalClose" aria-label="Close">\xD7</button>
          </div>
    ...
    function closeModal() {
      try {
        window.BlundersTeacher?.stopTeacherJobPolling?.();
      } catch {
      }
    ...
    function challengeCurrentPuzzle() {
      return window.BlundersChallenge?.challengeCurrentPuzzle?.() || null;
    }
    function clearChallengeUi() {
      return window.BlundersChallenge?.clearChallengeUi?.();
    }
    function renderChallengePage() {
      return window.BlundersChallenge?.renderChallengePage?.() || `<div class="bl-card">Challenge module not loaded.</div>`;
    }
    function renderLeaderboardPage() {
      return window.BlundersChallenge?.renderLeaderboardPage?.() || `<div class="bl-card">Leaderboard module not loaded.</div>`;
    }
    ...
    function renderTeacherSidebar() {
      return window.BlundersTeacher?.renderTeacherSidebar?.() || `<aside class="bl-sidebar"><div class="bl-side-title">\u{1F4A5} Blunders</div><div class="bl-side-sub">Teacher mode</div></aside>`;
    }
    function renderTeacherStudentsPage() {
      return window.BlundersTeacher?.renderTeacherStudentsPage?.() || `<div class="bl-card">Teacher module not loaded.</div>`;
    }
    function renderTeacherAllBlundersPage() {
      return window.BlundersTeacher?.renderTeacherAllBlundersPage?.() || `<div class="bl-card">Teacher module not loaded.</div>`;
    }
    function renderTeacherMasterGamePage() {
      return window.BlundersTeacher?.renderTeacherMasterGamePage?.() || `<div class="bl-card">Teacher module not loaded.</div>`;
    }
    ...
    async function teacherLoad(tab) {
      const m = requireTeacherModule();
      return m ? m.teacherLoad?.(tab) : void 0;
    }
    async function teacherSaveStudentSettings() {
      const m = requireTeacherModule();
      return m ? m.teacherSaveStudentSettings?.() : void 0;
    }
    async function teacherSaveMasters() {
      const m = requireTeacherModule();
      return m ? m.teacherSaveMasters?.() : void 0;
    }
    async function teacherSaveMasterConfig() {
      const m = requireTeacherModule();
      return m ? m.teacherSaveMasterConfig?.() : void 0;
    }
    async function teacherSyncStudent(studentId, hkDayKey, force) {
      const m = requireTeacherModule();
      return m ? m.teacherSyncStudent?.(studentId, hkDayKey, force) : void 0;
    }
    async function teacherHistoryScanStudent(studentId, historyGames, force) {
      const m = requireTeacherModule();
      return m ? m.teacherHistoryScanStudent?.(studentId, historyGames, force) : void 0;
    }
    async function teacherSyncMaster(masterId, hkDayKey, force) {
      const m = requireTeacherModule();
      return m ? m.teacherSyncMaster?.(masterId, hkDayKey, force) : void 0;
    }
    async function teacherHistoryScanMaster(masterId, historyGames, force) {
      const m = requireTeacherModule();
      return m ? m.teacherHistoryScanMaster?.(masterId, historyGames, force) : void 0;
    }
    async function teacherTagPuzzles(scope, recompute) {
      const m = requireTeacherModule();
      return m ? m.teacherTagPuzzles?.(scope, recompute) : void 0;
    }
    async function teacherLoadTagStats() {
      const m = requireTeacherModule();
      return m ? m.teacherLoadTagStats?.() : void 0;
    }
    async function teacherBulkSyncSelected(force) {
      const m = requireTeacherModule();
      return m ? m.teacherBulkSyncSelected?.(force) : void 0;
    }
    async function teacherBulkCompleteSelected() {
      const m = requireTeacherModule();
      return m ? m.teacherBulkCompleteSelected?.() : void 0;
    }
    async function teacherBulkHistoryScanSelected(force) {
      const m = requireTeacherModule();
      return m ? m.teacherBulkHistoryScanSelected?.(force) : void 0;
    }
    ...
    function renderInlineResultPanel(scope) {
      ...
      const title = verdict === "best" ? "Best Move" : verdict === "good" ? "Good move" : verdict === "blunder" ? "STILL BLUNDER!!" : "Result";
      const sub = verdict === "best" ? "Perfect. You found the best move." : verdict === "good" ? "Correct, but not the best." : verdict === "blunder" ? "Try again." : "Your result will appear here.";
      const iconSrc = verdict === "best" ? "/application/Sign/Best_move.jpeg" : verdict === "good" ? "/application/Sign/Good_move.jpeg" : verdict === "blunder" ? "/application/Sign/Blunder_move.jpeg" : "";
      ...
      const nextBtn = canNext ? `<button class="btn btn-secondary" type="button" data-bl-inline-next="${isMaster ? "master" : isChallenge ? "challenge" : "blunder"}">${isMaster ? "Next" : isChallenge ? "Next" : STATE.mode === "practice" ? "Next (Random)" : "Next"}</button>` : "";
      return `
    ...
    async function challengeLoadLeaderboard() {
      return window.BlundersChallenge?.challengeLoadLeaderboard?.();
    }
    async function challengeStartOrRestart() {
      return window.BlundersChallenge?.challengeStartOrRestart?.();
    }
    async function submitChallengeMoveUci(uci, revealBest) {
      return window.BlundersChallenge?.submitChallengeMoveUci?.(uci, revealBest);
    }
    function render() {
      ...
      const content = STATE.page === "home" ? renderHomePage() : STATE.page === "blunder" ? renderBlunderPage() : STATE.page === "masterGame" ? renderStudentMasterGamePage() : STATE.page === "review" ? renderReviewPage() : STATE.page === "challenge" ? renderChallengePage() : STATE.page === "leaderboard" ? renderLeaderboardPage() : renderSettingsPage();
      root.innerHTML = `
      <div class="bl-app">
        ${renderSidebar()}
        <main class="bl-main">
    ...
    function initBlunders() {
      ...
      root.addEventListener("click", async (ev) => {
        ...
        if (t?.closest?.("[data-bl-teacher-job-close]")) {
          try {
            window.BlundersTeacher?.teacherJobClose?.();
          } catch {
            closeModal();
          }
          return;
        ...
        if (t?.closest?.("[data-bl-teacher-job-refresh]")) {
          try {
            await window.BlundersTeacher?.teacherJobRefresh?.();
          } catch (e) {
        ...
        if (t?.closest?.("[data-bl-teacher-job-cancel]")) {
          try {
            await window.BlundersTeacher?.teacherJobCancel?.();
          } catch (e) {
            console.error("Job cancel failed:", e);
          }
          return;
        }
        ...
        if (t?.closest?.("[data-bl-teacher-all-stats]")) {
          try {
            return window.BlundersTeacher?.teacherLoadAllBlundersStorageStats?.();
          } catch {
          }
          return;
        }
        ...
        if (allToggle) {
          ...
          try {
            return window.BlundersTeacher?.teacherAllToggleBucket?.(key);
          } catch {
          }
          return;
        }
        ...
        if (allPrev) {
          ...
          try {
            return window.BlundersTeacher?.teacherAllPrev?.(key);
          } catch {
          }
          return;
        }
        ...
        if (allNext) {
          ...
          try {
            return window.BlundersTeacher?.teacherAllNext?.(key);
          } catch {
          }
          return;
        }
        ...
        if (allGo) {
          ...
          try {
            return window.BlundersTeacher?.teacherAllGo?.(key);
          } catch {
          }
          return;
        }
        ...
        if (t?.closest?.("[data-bl-teacher-apply-thr-selected]")) {
          const v = Number(STATE.teacher.bulkThreshold || 1) || 1;
          const selected = new Set(Array.isArray(STATE.teacher.selectedIds) ? STATE.teacher.selectedIds.map(String) : []);
          if (!selected.size) {
            STATE.teacher.error = "Please select at least one student.";
            render();
        ...
        if (t?.closest?.("[data-bl-teacher-save-students]")) {
          try {
            await teacherSaveStudentSettings();
            STATE.teacher.error = "";
          } catch (e) {
            STATE.teacher.error = String(e?.message || e);
          }
        ...
        if (t?.closest?.("[data-bl-go-blunder]")) {
          setBlunderModePending();
          return setPage("blunder");
        }
        ...
        if (hpd) {
          ...
          try {
            window.BlundersStudent?.resetReviewUi?.();
          } catch {
          }
          openHomePracticeModal();
          return;
        ...
        if (rvT) {
          ...
          try {
            return window.BlundersStudent?.reviewToggleBucket?.(key);
          } catch {
          }
          return;
        }
        ...
        if (rvP) {
          ...
          try {
            return window.BlundersStudent?.reviewPrev?.(key);
          } catch {
          }
          return;
        }
        ...
        if (rvN) {
          ...
          try {
            return window.BlundersStudent?.reviewNext?.(key);
          } catch {
          }
          return;
        }
        ...
        if (rvG) {
          ...
          try {
            return window.BlundersStudent?.reviewGo?.(key);
          } catch {
          }
          return;
        }
        ...
        if (inlineRetry) {
          ...
          if (scope === "master") {
            const pz2 = masterCurrentPuzzle();
            if (pz2) {
              STATE.uiBoard.masterFen = String(pz2.startFEN || "");
            }
            STATE.uiBoard.masterMoveUci = "";
            STATE.uiBoard.masterVerdict = "";
            STATE.uiBoard.masterBestMoveUci = "";
          ...
          if (scope === "challenge") {
            const pz2 = challengeCurrentPuzzle();
            if (pz2) STATE.uiBoard.challengeFen = String(pz2.startFEN || "");
            STATE.uiBoard.challengeMoveUci = "";
            STATE.uiBoard.challengeVerdict = "";
            STATE.uiBoard.challengeBestMoveUci = "";
            STATE.selectedFrom = null;
        ...
        if (openAll) {
          ...
          <div style="display:flex; gap:12px; flex-wrap:wrap; align-items:flex-start;">
            ...
            <div style="min-width:240px; max-width:520px;">
              ...
              ${tags.length ? `<div style="margin-top:10px; display:flex; gap:6px; flex-wrap:wrap;">${tags.map((t2) => `<span class="bl-badge" style="background:#eef2ff; color:#3730a3;">${escapeHtml(t2)}</span>`).join("")}</div>` : ``}
      ...
      root.addEventListener("change", (ev) => {
        ...
        if (rt) {
          ...
          try {
            window.BlundersStudent?.reviewSetTheme?.(v);
          } catch {
          }
          return;
        }
      ...
      root.addEventListener("input", (ev) => {
        ...
        if (rj) {
          ...
          try {
            window.BlundersStudent?.reviewSetJump?.(key, String(rj.value || ""));
          } catch {
          }
          return;
        }
        ...
        if (aj) {
          ...
          try {
            window.BlundersTeacher?.teacherAllSetJump?.(key, String(aj.value || ""));
          } catch {
          }
          return;
        }
        ...
        if (thrEl) {
          ...
          if (!STATE.teacher.edits.student[sid]) STATE.teacher.edits.student[sid] = {};
          STATE.teacher.edits.student[sid].thresholdPoints = Number.isFinite(v) ? v : 1;
          return;
        }
        ...
        if (mt) {
          ...
          if (!STATE.teacher.edits.masterCfg) STATE.teacher.edits.masterCfg = {};
          STATE.teacher.edits.masterCfg.thresholdPoints = Number.isFinite(v) ? v : 1;
          return;
        }
      ...
      function applyTeacherAllTagSearchFilter() {
        try {
          const root2 = document.getElementById("blundersRoot");
          if (!root2) return;
          const input = root2.querySelector("[data-bl-teacher-all-tag-search]");
          const select = root2.querySelector("[data-bl-teacher-all-tag]");
          const dl = root2.querySelector("#blTeacherAllTagList");
          ...
          if (!tagCounts) return;
          const entries = Object.entries(tagCounts).map(([k, v]) => ({ k: String(k), n: Number(v || 0) || 0 })).filter((x) => x.k && x.k !== "any").filter((x) => !q || x.k.toLowerCase().includes(q)).sort((a, b) => b.n - a.n || a.k.localeCompare(b.k)).slice(0, 200);
