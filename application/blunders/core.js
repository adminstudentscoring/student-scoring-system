(() => {
  // application/blunders/src/core-legacy.js
  (function() {
    ...
    const STATE = {
      page: "home",
      // 'home' | 'blunder' | 'review' | 'masterGame' | 'challenge' | 'leaderboard' | 'settings'
      mode: "pending",
      // 'pending' | 'practice'
      me: null,
      data: null,
      ...
      // 'board' | 'general'
      reviewDuration: "all",
      // 'week' | 'month' | 'halfYear' | 'year' | 'all'
      teacherTab: "students",
      // 'students' | 'masterGame' | 'settings'
      // Student Master Game
      master: {
        loading: false,
        error: "",
      ...
      teacher: {
        ...
        allBlunders: [],
        masterConfig: { maxGamesPerDay: 10, thresholdPoints: 1 },
        ...
        // week | month | halfYear | year | all
        allRating: "any",
        // any | 100-400 | 401-700 | 701-1000 | 1001-1500 | 1501-2000 | 2001-2300 | 2201-2500 | 2501-2800 | 2801-3000 | 3001up
        allTag: "any",
        // any | <tag>
        allUi: {
          pageSize: 50,
          ...
          storageStats: null,
          tagCounts: null,
          // { tag: count, ... } (top tags for current filters)
          buckets: {
            missMate: { open: false, page: 1, totalPages: 1, total: 0, entries: [], jump: "", loading: false, error: "" },
            d1: { open: false, page: 1, totalPages: 1, total: 0, entries: [], jump: "", loading: false, error: "" },
            d2: { open: false, page: 1, totalPages: 1, total: 0, entries: [], jump: "", loading: false, error: "" },
      ...
      homeRecent: {
        ...
        error: "",
        games: [],
        // [{ url, endTime, timeClass, pgn, fens, movesSan, blunders }]
        selectedGameIdx: 0,
        plyIdx: 0
      },
      homeAi: {
      ...
      ui: {
        ...
        reviewUi: {
          ...
          cacheKey: "",
          cache: null,
          // { totalAll, totalFiltered, counts, buckets: {key: puzzles[]} }
          buckets: {
    ...
    function getStudentPasswordQueryWith(extraParams) {
      const base = getStudentPasswordQuery();
      const parts = [];
    ...
    function puzzleTimeMs(p) {
      ...
      const endMs = Number.isFinite(end) && end > 0 ? end * 1e3 : 0;
    ...
    function reviewDurationStartMs(key) {
      ...
      if (k === "week") return Date.now() - 7 * 24 * 3600 * 1e3;
      if (k === "month") return Date.now() - 30 * 24 * 3600 * 1e3;
      if (k === "halfYear") {
