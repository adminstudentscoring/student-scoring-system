// Chess Light routes (Stage library)
"use strict";
...
async function ensureClSchema(pool) {
  await pool.query(`
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS chess_light_stages_org_idx ON chess_light_stages(org_id);`);
  await pool.query(`CREATE INDEX IF NOT EXISTS chess_light_stages_org_diff_idx ON chess_light_stages(org_id, difficulty);`);
}

function registerChessLightRoutes(app: any, deps: any): void {
  if (!app) throw new Error("registerChessLightRoutes: missing app");
  ...
  async function requireDbReady(res) {
    ...
    try {
      ...
      await ensureClSchema(pool);
      return true;
    } catch (e) {
      console.error("[chess-light] ensure schema failed:", e);
  ...
  if (authenticateUser && authorizeRole && requireOrganizationAccess) {
    app.get(
      "/api/teachers/chess-light/stages",
      authenticateUser,
      ...
      async (req, res) => {
        ...
        try {
          ...
          console.error("[chess-light] list stages error:", e);
    ...
    app.get(
      "/api/teachers/chess-light/stages/:stageId",
      authenticateUser,
      authorizeRole("teacher"),
      requireOrganizationAccess,
      async (req, res) => {
        ...
        try {
          ...
          console.error("[chess-light] get stage error:", e);
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        try {
          ...
          const r = await pool.query(
            `INSERT INTO chess_light_stages(org_id, difficulty, stage_no, config, created_by)
             VALUES ($1, $2, $3, $4, $5)
          ...
          console.error("[chess-light] create stage error:", e);
    ...
    app.patch(
      ...
      async (req, res) => {
        ...
        try {
          ...
          console.error("[chess-light] update stage error:", e);
          const msg = toCleanString(e?.message || e);
  ...
  app.get("/api/public/students/:id/chess-light/stages", async (req, res) => {
    if (!(await requireDbReady(res))) return;
    ...
    try {
      ...
      console.error("[chess-light] public list stages error:", e);
      return res.status(500).json({ ok: false, error: "Failed to list stages" });
    }
  });

  app.get("/api/public/students/:id/chess-light/stages/:stageId", async (req, res) => {
    if (!(await requireDbReady(res))) return;
    const auth = await requirePublicStudent(req, res);
    if (!auth) return;
    try {
      ...
      console.error("[chess-light] public get stage error:", e);
      return res.status(500).json({ ok: false, error: "Failed to load stage" });
    }
  });
}

module.exports = { registerChessLightRoutes };
