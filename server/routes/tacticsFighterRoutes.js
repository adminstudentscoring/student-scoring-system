...
function registerTacticsFighterRoutes(app, deps) {
  ...
  async function requireDbReady(res) {
    ...
    try {
      ...
      res.status(500).json({ ok: false, error: 'DB schema not ready' });
      return false;
    }
  }
      ...
      async (req, res) => {
        ...
        try {
          ...
          const cats = await pool.query(
            `SELECT id, name, created_at, updated_at FROM tactics_fighter_categories WHERE org_id = $1 ORDER BY name ASC`,
            [orgId]
          );
          const topics = await pool.query(
            `SELECT id, category_id, name, created_at, updated_at FROM tactics_fighter_topics WHERE org_id = $1 ORDER BY name ASC`,
            [orgId]
          );
          const subs = await pool.query(
            `SELECT id, topic_id, name, created_at, updated_at FROM tactics_fighter_subtopics WHERE org_id = $1 ORDER BY name ASC`,
            [orgId]
          );
          ...
          return res.status(500).json({ ok: false, error: "Failed to load builder tree" });
        }
      }
    );
      ...
      async (req, res) => {
        ...
        try {
          ...
          const r = await pool.query(
            `INSERT INTO tactics_fighter_categories(org_id, name, created_by)
             VALUES ($1, $2, $3)
             RETURNING id, name, created_at, updated_at`,
            [orgId, name, createdBy]
          );
          const row = r.rows?.[0];
          return res.json({ ok: true, category: { id: String(row.id), name: String(row.name), createdAt: row.created_at?.toISOString?.() || nowIso(), updatedAt: row.updated_at?.toISOString?.() || nowIso() } });
        } catch (e) {
