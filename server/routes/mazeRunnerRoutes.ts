...
async function ensureMrSchema(pool) {
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS maze_runner_stages_org_idx ON maze_runner_stages(org_id);`);
  await pool.query(`CREATE INDEX IF NOT EXISTS maze_runner_stages_org_diff_idx ON maze_runner_stages(org_id, difficulty);`);
}

function registerMazeRunnerRoutes(app: any, deps: any): void {
  if (!app) throw new Error("registerMazeRunnerRoutes: missing app");
  ...
  async function requireDbReady(res) {
    ...
    try {
      ...
      await ensureMrSchema(pool);
      return true;
    } catch (e) {
      console.error("[maze-runner] ensure schema failed:", e);
  ...
  if (authenticateUser && authorizeRole && requireOrganizationAccess) {
    app.get(
      "/api/teachers/maze-runner/stages",
      authenticateUser,
      ...
      async (req, res) => {
        ...
        try {
          ...
          console.error("[maze-runner] list stages error:", e);
    ...
    app.get(
      ...
      async (req, res) => {
        ...
        try {
          ...
          console.error("[maze-runner] get stage error:", e);
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        try {
          ...
          const r = await pool.query(
            `INSERT INTO maze_runner_stages(org_id, difficulty, stage_no, config, created_by)
             VALUES ($1, $2, $3, $4, $5)
          ...
          console.error("[maze-runner] create stage error:", e);
          return res.status(500).json({ ok: false, error: "Failed to create stage" });
        }
      }
    );
  ...
  app.get("/api/public/students/:id/maze-runner/stages", async (req, res) => {
    if (!(await requireDbReady(res))) return;
    const pub = await requirePublicStudent(req, res);
    if (!pub) return;
    try {
      const difficulty = normalizeDifficulty(req.query?.difficulty || "easy");
      const r = await pool.query(
      ...
      console.error("[maze-runner] public list stages error:", e);
      return res.status(500).json({ ok: false, error: "Failed to list stages" });
    }
  });

  app.get("/api/public/students/:id/maze-runner/stages/:stageId", async (req, res) => {
    if (!(await requireDbReady(res))) return;
    const pub = await requirePublicStudent(req, res);
    if (!pub) return;
    try {
      ...
      const r = await pool.query(
        `SELECT id, difficulty, stage_no, config, created_at, updated_at
         ...
         WHERE org_id = $1 AND id = $2
         LIMIT 1`,
        [String(pub.orgId), String(stageId)]
      );
      const row = r.rows?.[0];
      if (!row) return res.status(404).json({ ok: false, error: "Stage not found" });
      return res.json({
      ...
      console.error("[maze-runner] public get stage error:", e);
      return res.status(500).json({ ok: false, error: "Failed to load stage" });
    }
  });
}

module.exports = { registerMazeRunnerRoutes };
