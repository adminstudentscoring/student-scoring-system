// Backup Generator for Pets Data
// Run this script to generate pets-data.txt backup file

// Load pets data generator
const fs = require('fs');
const path = require('path');

// Import the pets data generator logic
...
function generateSkill(tier, element, petIndex) {
  ...
  const effectValue = 1.5 + (petIndex % 10) * 0.1;
  const cooldown = 3 + (petIndex % 8);
...
function generatePets() {
  ...
  for (let typeIndex = 0; typeIndex < 20; typeIndex++) {
    ...
    const baseHP = 500 + (typeIndex * 35);
    const baseAttack = 50 + (typeIndex * 3.5);
    const baseRecovery = 20 + (typeIndex * 2);
    const hasLeaderSkill = typeIndex < 5;
...
const backupContent = JSON.stringify(pets, null, 2);
fs.writeFileSync('pets-data.txt', backupContent, 'utf8');
console.log(`Backup file created: pets-data.txt (${pets.length} pets)`);
